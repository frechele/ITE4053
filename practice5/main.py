...
def run_experiment(lr: float, train_dataset: Dataset, test_dataset: Dataset, f):
    global W1, b1, W2, b2, vdW1, vdb1, vdW2, vdb2

    np.random.seed(12345)

    W1 = np.random.normal(size=(2, 1)) * np.sqrt(2. / 2)
    b1 = np.zeros((2, 1))
    W2 = np.random.normal(size=(1, 2)) * np.sqrt(2. / 1)
    b2 = np.zeros((1, 1))
    ...
    for k in range(1, 1+K):
        ...
        if k % K_print == 0 or k == 1:
            f.write('W1 {} b1 {} '.format(W1.reshape(-1), b1.reshape(-1)))
            f.write('W2 {} b2 {}'.format(W2.reshape(-1), b2.reshape(-1)))
